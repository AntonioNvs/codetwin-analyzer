    ...
    def search_repositories(
        ...
        for attempt in range(max_retries):
            ...
            if attempt < max_retries - 1:
                ...
                print(f"{error_msg} Tentando novamente em {sleep_time} segundos... (Tentativa {attempt + 1}/{max_retries})")
                time.sleep(sleep_time)
            else:
                raise SEARTAPIError(f"Falha ao buscar repositórios após {max_retries} tentativas. Último erro: {error_msg}")
            
        try:
            data = response.json()
        except ValueError:
            raise SEARTAPIError(f"A API retornou uma resposta não-JSON (Status {response.status_code}): {response.text}")
